...
function Nav(){
  ...
  return (
    <div className="w-full h-25 flex justify-between items-center px-5 md:px-8">
      ...
      <form className=" w-[45%] h-15 bg-white flex items-center px-5 gap-5 
      rounded-md shadow-md md:w-[50%]" onSubmit={(e)=>e.preventDefault()}>
       ...
       <div className="w-15 h-15 bg-white flex justify-center items-center rounded-md 
       shadow-xl relative" onClick={()=>{
        setShowCart(true)
       }}>
        <span className='absolute top-0 right-2 text-green-500 font-bold text-[18px]'>{items.length}</span>
        <LuShoppingBag className='w-7.5 h-7.5 text-green-500 cursor-pointer' />
      </div>
    </div>
  );
}
